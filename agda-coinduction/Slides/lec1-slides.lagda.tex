...
\footnote{Enabled by \texttt{\{-\# OPTION -{}-guardedness \#-\}}}
...
\begin{code}
  ...
  fromStream' {A} xs = xs .head ∷
    (λ where .force → fromStream' (xs .tail))
\end{code}
...
\alert{Sized types}\footnote{Enabled by \texttt{\{-\# OPTION -{}-sized-types \#-\}}} are an alternative to the syntactic guardedness checker that annotates the size of expressions in their types.
